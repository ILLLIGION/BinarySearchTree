...
struct l_err : public std::logic_error
{
public:
	l_err(const std::string& data) : logic_error(data) {}
};
...
public:
	...
	auto insert(const T& value) -> bool
	{
		...
		while (!foundPlace)
		{
			if (value == thisNode->value)
				throw l_err("element already exists");
			if (value < thisNode->value)
			{
				if (!thisNode->left_)
				{
	...
	while (1)
	{
		if (value == thisNode->value)
		{
			...
			if (thisNode->left_)
				thisNode = thisNode->left_;
			else { throw l_err("element not found"); }
		else {
			if (thisNode->right_)
				thisNode = thisNode->right_;
			else
				throw l_err("element not found");
		}
	}
	};
